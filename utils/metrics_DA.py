...
#高斯核函数的核心思想是将每一个样本点映射到一个无穷维的特征空间，从而使得原本线性不可分的数据线性能够线性可分。
    def L2dis(self,source, target):
        ...
        # print('L2_distance1', L2_distance.shape)
        L2_distance=L2_distance.sum(2)
        # L2_distance=torch.exp(L2_distance)-1
        # print('L2_distance2',L2_distance.shape)
        return L2_distance
...
#条件
    def conditionalnew(self, source, target, s_label, t_label,DEVICE, kernel_mul=2.0, kernel_num=3, fix_sigma=None):
        ...
        var = torch.var(torch.cat([source,target],0).mean(1))
        # print(var.shape)
        # var=0
        #margin
        zeros = torch.zeros_like(source)
        ones = torch.ones_like(source)
        # zeros2 = torch.zeros_like(source)
        s_labelre = (target.repeat([1, source.shape[1], 1, 1])).float()
        margin_out_sim_out = source - 0.1
        ...
        unchgnum = (margin_out_sim_flag * (1 - s_labelre)).sum() + 1
        loss1 = (1 - s_labelre) * (torch.exp(margin_out_sim) - 1)
        sloss1 = loss1.sum() / unchgnum
        lossdist = ((s_labelre) * (torch.exp(-source)))
        chgnum = s_labelre.sum() + 1
        slossdist = lossdist.sum() / chgnum
        sloss = sloss1 + slossdist  # +(1/(var_mean+0.001)).mean()
        # print('sloss',sloss,"var",var)
        ##
        ...
        # loss = torch.sum(-2 * weight_onest * weight_st * ST)
        loss = torch.sum( -2 *weight_onest* weight_st * ST - weight_onett * weight_tt * TT - weight_oness* weight_ss * SS)
        return loss,var,sloss
